    ...
    while( 1 )
    {
        ...
        for( int i = 0; i < user_counter+1; ++i )
        {
            ...
            else if( fds[i].revents & POLLRDHUP )
            {
                ...
                // 这里有哈希的味道，定向检索已退出用户，并将其与末尾元素位置交换
            ...
            else if( fds[i].revents & POLLIN )  // 若该用户数据可读
            {
                int connfd = fds[i].fd;
                ...
                else
                {
                    ...
                    for( int j = 1; j <= user_counter; ++j )
                    {
                        ...
                        fds[j].events |= ~POLLIN;   // 将该客户端的可读信号关闭，有点像锁的感觉
                        fds[j].events |= POLLOUT;   // 打开客户端可写信号
                        users[fds[j].fd].write_buf = users[connfd].buf;
                    }
                }
            }
            // 这里实际上是用户接收到了其它用户发过来的消息，准备开始写，上面分支中实际上并没有对用户进行写
