...
void processpool< T >::setup_sig_pipe()
{
    ...
    /*设置信号处理函数*/
    addsig( SIGCHLD, sig_handler ); // 第十章末尾有，这个表示进程状态发生变化（退出或暂停）
    addsig( SIGTERM, sig_handler ); // 终止进程，kill命令默认发送的信号就是SIGTERM
    addsig( SIGINT, sig_handler );  // 键盘输入以中断进程
    addsig( SIGPIPE, SIG_IGN );     // 左边表示向管道或者socket中写入数据，右边表示ignore此信号
}
...
void processpool< T >::run_child()
{
                    ...
                    for( int i = 0; i < ret; ++i )
                    {
                        switch( signals[i] )
                        {
                            case SIGCHLD:   // 子进程状态发生变化（退出或暂停）
                            {
                                pid_t pid;
                                int stat;
                                // waitpid用于处理僵尸进程，WNOHANG参数表示该调用是非阻塞的
    ...
    close( pipefd );
    /*我们将这句话注释掉，以提醒读者：
    应该由m_listenfd的创建者来关闭这个文件描述符（见后文），
