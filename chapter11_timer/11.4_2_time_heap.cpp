...
public:
        ...
        : cur_size(size), capacity(capacity)
    {
        ...
        if (size != 0)
        {
            ...
            {
                // (cur_size-1)/2实际上是此种堆形式中最后一个父节点的下标
...
public:
    ...
    void add_timer(heap_timer *timer) throw(std::exception)
    {
        ...
        for (; hole > 0; hole = parent) // 注意hole的更新是在for循环中实现的
        {
            parent = (hole - 1) / 2;
...
private:
    ...
    void percolate_down(int hole)
    {
        ...
        {
            child = hole * 2 + 1;
            // 第一个条件相当于判断child的兄弟节点在取值范围以内，即child存在兄弟节点
